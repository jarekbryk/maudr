...
Educational resources for teaching enzyme kinetics in undergraduate courses have been recently summarised and discussed by Mak and colleagues (1). These resources tend to focus on the calculations of enzymatic transformations (2) or simulations of the reactions across a range of kinetic models (3), and are less concerned with the practical logistics of providing appropriate datasets to large cohorts of students and ensuring accurate marking of work where students perform calculations on and visualise enzyme kinetics.

maudr fills this gap by generating an arbitrary number of individualised student datasets in Microsoft Excel format, matching data typically acquired in an "enzyme inhibition" laboratory class, and automatically produces visual and numerical answers in PDF format to facilitate marking and feedback. The package requires only a basic ability to run R functions and, by default, provides widely used set of kinetic parameters for yeast alcohol dehydrogenase and its inhibitors (4).
...
In a typical use scenario, an instructor provides a list of students and maudr randomly assigns each student a substrate and an inhibitor from the default reaction parameters table, and then simulates absorbance vs time data for both an inhibited and non-inhibited reaction across substrate concentrations from 0 and 160 mM, using the enzyme's Kcat, Km and Vmax (4). A small, user-controllable amount of noise is added to each value in the generated dataset to simulate experimental variation (this option can be switched off). Each student receives a single Excel file with individualised data, with information of the type of inhibition hidden.
